...
async def hybrid_parsing_single_video(
    ...
    # [中文]
    ### 用途:
    - 该接口用于解析抖音/TikTok/Bilibili/YouTube单一视频的数据。
    ### 参数:
    - `url`: 视频链接、分享链接、分享文本。
    ### 返回:
    - `data`: 视频数据。
    ...
    # [English]
    ### Purpose:
    - This endpoint is used to parse data of a single Douyin/TikTok/Bilibili/YouTube video.
    ### Parameters:
...
# 更新Cookie
@router.post(
    "/update_cookie", response_model=ResponseModel, summary="更新Cookie/Update Cookie"
)
async def update_cookie_api(
    request: Request,
    service: str = Body(example="douyin", description="服务名称/Service name"),
    cookie: str = Body(
        example="YOUR_NEW_COOKIE", description="新的Cookie值/New Cookie value"
    ),
):
    """
    # [中文]
