...
def error_do(reason: str, value: str) -> None:
    ...
    put_table(
        [
            [ViewsUtils.t("原因", "reason"), ViewsUtils.t("输入值", "input value")],
            [reason, value],
        ]
    )
...
def parse_video():
    ...
    for url in url_lists:
        ...
        try:
            data = asyncio.run(
                HybridCrawler.hybrid_parsing_single_video(url, minimal=True)
            )
        ...
        # 创建一个视频/图集的公有变量
        url_type = (
            ViewsUtils.t("视频", "Video")
            if data.get("type") == "video"
            else ViewsUtils.t("图片", "Image")
        )
        ...
        table_list = [
            ...
            [
                ViewsUtils.t("作者昵称", "Author nickname"),
        ...
        if url_type == ViewsUtils.t("视频", "Video"):
            ...
            # 如果是抖音视频，添加评论下载选项
            if platform == "douyin" and data.get("aweme_id"):
                aweme_id = data.get("aweme_id")
                table_list.insert(
                    8,
                    [
                        ViewsUtils.t("下载评论CSV", "Download Comments CSV"),
                        put_link(
                            ViewsUtils.t("点击下载", "Click to download"),
                            f"/api/douyin/comments/export?aweme_id={aweme_id}&max_comments=100",
                            new_window=True,
                        ),
                    ],
                )
        ...
        # 如果是图片/If it's image
            ...
            table_list.insert(
                ...
                [
                    ViewsUtils.t(
                        "图片打包下载-无水印", "Download images ZIP-No-Watermark"
                    ),
                    put_link(
                        ViewsUtils.t("点击下载", "Click to download"),
                        f"/api/download?url={url}&prefix=true&with_watermark=false",
            ...
            # 添加图片信息
            no_watermark_image_list = data.get("image_data").get(
            ...
            for image in no_watermark_image_list:
                table_list.append(
                    [
                        ViewsUtils.t(
                            "图片预览(如格式可显示): ",
                            "Image preview (if the format can be displayed):",
                        ),
                ...
                table_list.append(
                    [
                        ...
                        put_link(
                            ViewsUtils.t("⬆️点击打开图片⬆️", "⬆️Click to open image⬆️"),
                            image,
                            new_window=True,
                        ),
                    ]
        ...
        with use_scope(str(url_index)):
            ...
            put_info(
                ViewsUtils.t(
                    f"正在解析第{url_index}/{url_count}个链接: ",
    ...
    with use_scope("result"):
        ...
        put_markdown(
            ViewsUtils.t(
                "**以下是您输入的所有链接：**",
                "**The following are all the links you entered:**",
            )
        )
        put_code("\n".join(url_lists))
        # 解析结束时间
        ...
        # 放置一个按钮，点击后跳转到顶部
        put_button(
            ViewsUtils.t("回到顶部", "Back to top"),
            onclick=lambda: scroll_to("1"),
            color="success",
            outline=True,
        )
        # 返回主页链接
