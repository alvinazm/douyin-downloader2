        ...
        with use_scope("main"):
            ...
            put_row(
                [
                    ...
                    put_button(
                        self.utils.t("关于", "About"),
                        onclick=lambda: about_pop_window(),
                        link_style=True,
                        small=True,
                    ),
                ]
            ...
            select_options = select(
                self.utils.t(
                    "请在这里选择一个你想要的功能吧 ~",
                    "Please select a function you want here ~",
                ),
                required=True,
                options=options,
                help_text=self.utils.t(
                    "📎选上面的选项然后点击提交",
                    "📎Select the options above and click Submit",
                ),
            )
            # 根据输入运行不同的函数
            if select_options == options[0]:
                ...
                put_markdown(
                    self.utils.t(
                        "暂未开放，敬请期待~",
                        "Not yet open, please look forward to it~",
                    )
                )
