...
# Vue Frontend
# Search for frontend directory in parent directories
script_dir = os.path.dirname(os.path.abspath(__file__))
search_path = script_dir

# Search up to 3 levels up for frontend directory
for _ in range(3):
    frontend_path = os.path.join(search_path, "frontend", "dist")
    if os.path.exists(frontend_path):
        break
    search_path = os.path.dirname(search_path)
else:
    # If not found, use a relative path from current working directory
...
if os.path.exists(frontend_path):
    ...
    # Catch-all route for SPA (Single Page Application) - return index.html for all non-API routes
    @app.get("/{full_path:path}")
    async def serve_spa(full_path: str):
        # Don't intercept API routes and other known routes
        if (
            full_path.startswith("api")
            or full_path.startswith("docs")
            or full_path.startswith("redoc")
            or full_path.startswith("openapi")
            or full_path.startswith("pywebio")
        ):
