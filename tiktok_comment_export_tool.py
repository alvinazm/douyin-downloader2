...
TikTok视频评论导出工具
使用方法：
    python3 tiktok_comment_export_tool.py <aweme_id> [max_comments]

示例：
    python3 tiktok_comment_export_tool.py 7304809083817774382 100
"""
...
class TikTokCommentExporter:
    ...
    async def fetch_all_comments(self, aweme_id: str, max_comments: int = None):
        ...
        print(f"开始获取TikTok评论... (aweme_id: {aweme_id})")

        while True:
            try:
                print(f"获取第 {total_count // count + 1} 页评论...")
                response = await self.crawler.fetch_post_comment(
    ...
    async def export_comments_to_csv(
        ...
        """
        if not output_file:
            output_file = f"download/comments/tiktok_comments_{aweme_id}.csv"
...
async def main():
    if len(sys.argv) < 2:
        print(
            "使用方法: python3 tiktok_comment_export_tool.py <aweme_id> [max_comments] [output_file]"
        )
        print("示例: python3 tiktok_comment_export_tool.py 7304809083817774382 100")
        sys.exit(1)
    ...
    exporter = TikTokCommentExporter()
